...
export const state: RuntimeState = {
	...
	ui: {
		...
		overlays: {
			...
			prompts: createDropdownOverlayState(),
...
export function getStatusReport(): string {
	...
	return [
		...
		`- viewport drag direction: ${state.shell.viewportScrollDirectionInverted ? "inverted" : "natural"}`,
		...
		`- prompts overlay: ${state.ui.overlays.prompts.visible}`,
		...
		`- loaded prompts: ${state.pi?.getCommands().filter(c => c.source === "prompt").length ?? "?"}`,
